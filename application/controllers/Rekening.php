...
class Rekening extends CI_Controller {
  ...
  public function __construct(){
	...
	public function index()
	{
     $this->session_check();
	  ...
	  $jenis_belanja = 2;
      if($role =='Admin'){
        $data = $this->M_Rekening->get_rekening_admin($jenis_belanja);
      }else{
  ...
  function data_temp_pengadaan(){
    ...
    function data_rekening(){
				...
				$jenis_belanja = 2;
				if($role =='Admin'){
					$baru = $this->M_Rekening->get_rekening_admin($jenis_belanja);
				}else{
					$baru = $this->M_Rekening->get_rekening_admin($id_user,$jenis_belanja);
    ...
    function update_temp_pengadaan(){
  	...
  	function update_pengadaan(){
		...
		function save_pengadaan(){
			...
			//belanja barang dan jasa 1 //belanja Rekening 2
			$jenis_belanja = 2;
		...
		function data_detail_pengadaan(){
			// $id_user =  $this->session->userdata('id_user');
			// $id_uraian = 2;
			$baru=$this->M_Rekening->tampil_detail();
			$data['data']=$baru;
			echo json_encode($data);
        ...
        echo json_encode($data);
			...
			function CetakUsulan(){
				...
				$this->load->view("Cetak/export_usulan_Rekening",$data);
