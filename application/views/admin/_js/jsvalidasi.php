...
$(document).ready(function(){
        ...
        ptable = $('#PenggunaTable').DataTable( {
            ...
            'responsive': true, 
            ajax:{
                url   : '<?php echo base_url('Master/ambil_capaian')?>',
                  ...
                  var hitung = response.data.length;
                    if (hitung>0) {
                      for(var x in response.data){
                        no = '<input type="checkbox" id="pilih" value='+response.data[x].id_capaian+' onChange="pilihan('+response.data[x].id_capaian+')">';
                        ...
                        row.push({
                          ...
                          'denum'     : response.data[x].denum,
                          'plan'     : response.data[x].plan,
                          'do'     : response.data[x].do,
                          'study'     : response.data[x].study,
            ...
            columns : [ 
              ...
              {'data': 'status', 'render':
                function (data, type, full) {
                  if(full.status == 0){
                      return "<span class='badge badge-info'>belum divalidasi</span>";
                  }
                  else if(full.status == 1){
                    return "<span class='badge badge-success'>diterima</span>";
                  }
                  else{
                    return "<span class='badge badge-danger'>validasi ditolak</span>";
                  }
