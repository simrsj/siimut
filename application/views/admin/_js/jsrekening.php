...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        ptable = $('#RekeningTable').DataTable( {
            'processing'	: true,
            'scrollX'   :true,
        
            //'serverSide'	: true,
            ajax:{
                url   : '<?php echo base_url('Rekening/data_rekening')?>',
                    ...
                    if (hitung>0) {
                      for(var x in response.data){
                        // <button onClick="DeletePengadaan('+response.data[x].id_pengadaan+')" name="btn_delete" class="btn btn-danger btn-xs btn-flat" title="Hapus Data"><i class="fa fa-trash"></i></button><button onClick="KirimPersetujuan('+response.data[x].id_pengadaan+')" name="btn_validasi" class="btn btn-info btn-xs btn-flat" title="Kirim Ke RTP"><i class="fa fa-share-square" aria-hidden="true"></i></button> 
                             var link = response.data[x].id_uraian;
                            //  console.log(link);
                            ...
                            //  || id_user ==16
                            // if(role == 'Admin'){
                                button = '<button onClick="BukaEdit('+response.data[x].id_uraian+')" name="btn_edit" class="btn btn-warning btn-xs btn-flat" title="Edit Data"><i class="fa fa-edit"></i></button> <a href="<?php base_url();?>CetakUsulan/'+link+'" name="btn_cetak" class="btn btn-success btn-xs btn-flat"  title="Cetak Data"><i class="fas fa-download"></i></a>';
                            // }else{
                                // button = '<a href="<?php base_url();?>CetakUsulan/'+link+'" name="btn_cetak" class="btn btn-success btn-xs btn-flat"  title="Cetak Data"><i class="fas fa-print"></i></a>';

                            // }
                           
                            button = '<button onClick="BukaEdit('+response.data[x].id_uraian+')" name="btn_info" class="btn btn-info btn-xs btn-flat" title="Info"><i class="fas fa-info-circle"></i></button>';
                        row.push({
                          'no'                : i,
        ...
        dpttable = $('#DetailPengadaantempTable').DataTable( {
            ...
            //'serverSide'	: true,
            ajax:{
                 url   : '<?php echo base_url('Rekening/data_temp_pengadaan')?>',
                dataSrc : function(response){
                  var row = new Array();
                  //console.log()
                  var i = 1;
    ...
    //              
        ...
        function BukaEdit(x){
            ...
            //     ambileuraian();
                ...
                dptable.destroy();
                dptable = $('#DetailRekeningTable').DataTable( {
                'processing'	: true,
                'sScrollX'      : '100%',
                //'serverSide'	: true,
                ajax:{
                    type:"POST",
                    url   : '<?php echo base_url('Rekening/data_detail_pengadaan')?>',
                    data : {id:id},
                    dataSrc : function(response){
                        var row = new Array();
                        console.log(response);
                        ...
                        if (hitung>0) {
                            ...
                            $('#p_kode_pengadaan').html(response.data[x].kodering_uraian);
                            $('#e_unit_kerja').html((response.data[x].nama_uraian).toUpperCase());
            ...
            {'data': 'total_harga', 'render': 
             ...
             order: [[0, 'asc'],[1, 'asc']],
             rowGroup: {
                dataSrc: [ 'nomen_uraian', 'unit_kerja']
                },
                columnDefs: [ 
                    {targets: [0,1,2,3,7], visible: false},
                    // {  targets: 0, width: '20%' }, 
